...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    c3 = (int(x[0])+int(((int(x[2])-int(x[0])))/2), int(x[1])+int((int(x[3])-int(x[1]))/2))
    # print('plot_one_box_c1:', c1)
    # print('plot_one_box_c2:', c2)
    # print('plot_one_box_c3:', c3)
    # print('plot_one_box_label:', label)
    cv2.circle(img, c1, 15, (255, 0, 0), -1)
    cv2.circle(img, c2, 15, (0, 255, 0), -1)
    cv2.circle(img, (c1[0], c2[1]), 8, (0, 0, 255), -1)
    ...
    #     cv2.rectangle(img, c1, c2, color, -1, cv2.LINE_AA)  # filled
        ...
        # cv2.putText(img, str(c3), (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
        
def plot_point(img, x_list, y_list, line_thickness=None):
    # Plots one bounding box on image img
    # tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    # color = color or [random.randint(0, 255) for _ in range(3)]
    color_list=[(255,0,0),(0,255,0),(0,0,255)]
    cv2.circle(img,(max(x_list),max(y_list)), 15, (255, 0, 0), -1)#blue
    cv2.circle(img,(min(x_list),min(y_list)), 15, (0, 255, 0), -1)#green
    cv2.circle(img,(min(x_list),max(y_list)), 15, (0, 0, 255), -1)#red
    # cv2.rectangle(img, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)

def plot_all_point(img, x_list, y_list, idx, line_thickness=None):
    # Plots one bounding box on image img
    # tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    # color = color or [random.randint(0, 255) for _ in range(3)]
    color_list=[(255,0,0),(0,255,0),(0,0,255)]
    if len(x_list)==len(y_list):
        for i in range(len(x_list)):
            cv2.circle(img,(x_list[i], y_list[i]), 15, color_list[idx], -1)#blue
            # cv2.circle(img,(min(x_list),min(y_list)), 15, (0, 255, 0), -1)#green
            # cv2.circle(img,(min(x_list),max(y_list)), 15, (0, 0, 255), -1)#red
        # cv2.rectangle(img, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
def plot_cap_box(cap_x, cap_y, img, color=None, label=None, line_thickness=None):
    # Plots one bounding box on image img
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    # c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    cv2.rectangle(img, cap_x, cap_y, color, thickness=tl, lineType=cv2.LINE_AA)
