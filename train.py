...
# python train.py --img 1080 --batch 16 --epochs 50 --data data.yaml --weights yolov5s.pt
...
def train(hyp, opt, device, tb_writer=None):
    ...
    last = wdir / 'onion_last.pt'
    best = wdir / 'onion_best.pt'
    results_file = str(log_dir / 'qrcode_results.txt')
...
if __name__ == '__main__':
    ...
    #parser.add_argument('--weights', type=str, default='drone_best2.pt', help='initial weights path')
    parser.add_argument('--weights', type=str, default='yolov5s.pt', help='initial weights path')  
    #parser.add_argument('--weights', type=str, default='onion_best.pt', help='initial weights path')  
    parser.add_argument('--cfg', type=str, default='', help='model.yaml path')
    #parser.add_argument('--data', type=str, default='data/drone.yaml', help='data.yaml path')
    parser.add_argument('--data', type=str, default='data/qrcode_yolo_train.yaml', help='data.yaml path')
    parser.add_argument('--hyp', type=str, default='data/hyp.scratch.yaml', help='hyperparameters path')
    parser.add_argument('--epochs', type=int, default=12)#1200
    parser.add_argument('--batch-size', type=int, default=16, help='total batch size for all GPUs')
    #parser.add_argument('--img-size', nargs='+', type=int, default=[640, 640], help='[train, test] image sizes')
    parser.add_argument('--img-size', nargs='+', type=int, default=[300, 300], help='[train, test] image sizes')
    ...
    if opt.resume:  # resume an interrupted run
        ...
        if opt.name =='':
            log_dir = increment_dir(Path(opt.logdir) / 'exp', (opt.data.split('\\'))[-1].split('.')[0])  # runs/exp1
        else:
            log_dir = increment_dir(Path(opt.logdir) / 'exp', opt.name)  # runs/exp1_opt.name
